...
def run_task(logger, dm, criterion, models, task, batch_size_valid):
    logger.start_task()
...
def main():
    ...
    criterion = load_instance(**conf.criterion_eval)
    models = []
    for conf_model in conf.models_eval:
        models.append(load_instance(**conf_model))
    for task in conf.tasks_eval:
        run_task(logger, dm, criterion, models, task, conf.batch_size_valid)
